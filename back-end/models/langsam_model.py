...
OUTPUT_DIR = 'app/static/uploads/lang_segmented'
...
def draw_image(image, masks, boxes, labels, alpha=0.5):
...
def langsam_output_path(class_name, filepath, prompt):
    ...
    # Check and handle masks shape
    if len(masks.shape) == 3:  # Add batch dimension if missing
        masks = masks.unsqueeze(0)
    elif len(masks.shape) == 4 and masks.shape[1] == 1:  # Remove unnecessary channel dimension
...
def langsam_output(class_name, filepath, prompt):
    ...
    segmented_image = Image.fromarray(image_np)
...
class LangSAM:
    ...
    def save_segmented_image(self, image_pil, masks, boxes, filepath):
        # Convert PIL Image to numpy array for drawing
        image_np = np.array(image_pil)
